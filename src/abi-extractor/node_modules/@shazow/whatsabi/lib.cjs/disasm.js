...
const interestingOpCodes = new Set([
    ...
    opcodes_js_1.opcodes.CREATE,
    opcodes_js_1.opcodes.CREATE2,
]);
class Function {
    byteOffset;
    opTags;
...
function disasm(bytecode, config) {
    ...
    while (code.hasMore()) {
        ...
        if (inst === opcodes_js_1.opcodes.PUSH32) {
            ...
            p.eventCandidates.add((0, utils_js_1.bytesToHex)(lastPush32));
            continue;
        }
        if (inst === opcodes_js_1.opcodes.DELEGATECALL &&
            ...
            if ((0, opcodes_js_1.isPush)(code.at(-3))) {
                ...
                const addr = (0, utils_js_1.bytesToHex)(val.slice(val.length - 20), 20);
                if (!p.proxies.find(p => p instanceof proxies_js_1.FixedProxyResolver && p.resolvedAddress === addr)) {
        ...
        else if (((0, opcodes_js_1.isHalt)(code.at(-2)) || code.at(-2) === opcodes_js_1.opcodes.JUMP) &&
            runtimeOffset < pos &&
            maxJumpDest < pos) {
            boundaryPos = pos;
            break;
        ...
        if (currentFunction.opTags !== undefined) {
            ...
            if (interestingOpCodes.has(inst)) {
                ...
                if (inst === opcodes_js_1.opcodes.CREATE || inst === opcodes_js_1.opcodes.CREATE2) {
                    p.isFactory = true;
                }
                else if (inst === opcodes_js_1.opcodes.SSTORE) {
                    p.sstoreCount += 1;
                }
            }
        }
        if (code.at(-1) === opcodes_js_1.opcodes.CODECOPY &&
            (code.at(-2) === opcodes_js_1.opcodes.PUSH1 ||
                code.at(-2) === opcodes_js_1.opcodes.PUSH0 ||
                code.at(-2) === opcodes_js_1.opcodes.RETURNDATASIZE) &&
            (0, opcodes_js_1.isPush)(code.at(-3))) {
    ...
    if ((boundaryPos > 0 && p.proxies.length === 0)) {
        let endBoundary = undefined;
        const finalByte = code.bytecode.slice(-1)[0];
        if (!(0, opcodes_js_1.isHalt)(finalByte)) {
            const cborLength = valueToOffset(code.bytecode.slice(-2));
            endBoundary = -(cborLength + 4);
        }
        const auxData = (0, utils_js_1.bytesToHex)(code.bytecode.slice(boundaryPos, endBoundary));
        if (auxData.length >= 2 + 64) {
            for (const [slot, resolver] of Object.entries(proxies_js_1.slotResolvers)) {
                if (auxData.lastIndexOf(slot.slice(2)) === -1)
                    continue;
                p.proxies.push(resolver);
            }
        }

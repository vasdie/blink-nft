...
export class Program {
    ...
    proxies;
    isFactory; // CREATE or CREATE2 detected
    sstoreCount; // Number of SSTORE instructions detected, used to determine if this may be a destination contract (vs a proxy)
    ...
    while (code.hasMore()) {
        ...
        if (inst === opcodes.DELEGATECALL &&
            ...
            if (isPush(code.at(-3))) {
                ...
                const addr = bytesToHex(val.slice(val.length - 20), // Might be padded with zeros
                20);
        ...
        else if ((isHalt(code.at(-2)) || code.at(-2) === opcodes.JUMP) &&
            runtimeOffset < pos &&
            maxJumpDest < pos) {
